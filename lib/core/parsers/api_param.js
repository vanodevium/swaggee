...
const allowedValuesWithDoubleQuoteRegExp = new RegExp(/"[^"]*[^"]"/g);
const allowedValuesWithQuoteRegExp = new RegExp(/'[^']*[^']'/g);
const allowedValuesRegExp = new RegExp(/[^,\s]+/g);
...
function parse(content, source, defaultGroup) {
  ...
  if (allowedValues) {
    let regExp;
    if (allowedValues.charAt(0) === '"') regExp = allowedValuesWithDoubleQuoteRegExp;
    else if (allowedValues.charAt(0) === "'") regExp = allowedValuesWithQuoteRegExp;
    else regExp = allowedValuesRegExp;
  ...
  return {
    ...
    optional: !!(matches[5] && matches[5][0] === '['),
    field: matches[6],
