...
function detectLineEnding() {
  ...
  if (os.platform() === 'darwin') return '\r';
  return '\n';
}
 ...
 * @returns {{}|boolean} true = ok, but nothing todo | false = error | Object with parsed data and project-information.
 *          {
 *              data   : { ... }
 *              project: { ... }
 *          }
...
function parse(options) {
  ...
  try {
    ...
    if (parsedFiles.length) {
      ...
      const blocks = filter.process(parsedFiles, parsedFilenames);
 ...
 * Set package infos.
 *
 * @param {Object} [packageInfos]             Collected from swaggee.json / package.json.
