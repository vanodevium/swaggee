import { isObject, extend, each } from 'lodash';
...
class Worker {
  constructor(_app) {
    ...
    workers.forEach(function(worker) {
      if (isObject(app.workers[worker])) {
   ...
   */
  process(parsedFiles, parsedFilenames, packageInfos) {
    ...
    parsedFiles.forEach(function(parsedFile, fileIndex) {
      parsedFile.forEach(function(block) {
        if (!Object.keys(block.global).length && Object.keys(block.local).length) {
    ...
    each(this.workers, function(worker, name) {
      if (worker.preProcess) {
        ...
        extend(preProcessResults, result);
      }
    });
    each(this.workers, function(worker, name) {
